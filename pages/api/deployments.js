...
export default async function handler(req, res) {
    try {
        ...
        if (req.query.action === "getDeployments") {
            ...
            fs.writeFileSync("./nds_config.json", JSON.stringify(config, null, 4));
            console.log(logprefix(user) + "Created deployment: " + name + " with id: " + id);
            ...
            container.restart().then(res => {
                return res.send({ error: "Deployment has not been initialized yet! Failed to restart" });
            }).catch(err => {
            ...
            fs.writeFileSync("./nds_config.json", JSON.stringify(config, null, 4));
            console.log(logprefix(user) + "Deployment with id:" + id + " deleted!");
